...
public class OreGeneration {
    ...
    public static void oreGeneration(final BiomeLoadingEvent event) {
        if (!(event.getCategory().equals(Biome.Category.NETHER) || event.getCategory().equals(Biome.Category.THEEND))) {
            generateOre(event.getGeneration(), OreFeatureConfig.FillerBlockType.NATURAL_STONE, COBlocks.CINNABAR_ORE.get().defaultBlockState(),
                     1, 80, 12);
    ...
    private static void generateOre(BiomeGenerationSettingsBuilder set, RuleTest ft, BlockState st,
        ...
        set.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, Feature.ORE.configured(new OreFeatureConfig(ft, st, amount
